...
file_path = "C:\\Users\\Mustafa Ajnawala\\Downloads"
...
# Create a loop to first travers all the files in folder and then add then in the
#list and dictionary + also creation of the specific folder.
for file in files:

	# Split the name of file from dot
	filetype = file.split('.')[1]

	#will run if the fie type is not present before
...
# Create the loop for actually going through all the files and then
#shifting them to the respective file
for file in files:
	...
	if filetype in filetype_dict.keys():
		...
		# performs the actual file movement from source path to destination path
